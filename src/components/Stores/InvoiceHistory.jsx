    ...
    <div className="space-y-5">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Route: <span className="text-accent-600">{shop?.route}</span>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.length === 0 ? (
                ...
                filtered.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className={`group hover:bg-gray-50 transition-colors ${isDeleting ? 'bg-red-50' : ''}`}>
                      ...
                      <td className="table-cell text-xs text-gray-500">
                        {t.chequeNo
                          ? <span className="font-mono">{t.chequeNo}{t.bankName ? <span className="ml-1 text-gray-400">/ {t.bankName}</span> : null}</span>
                          : '—'}
                      </td>
                      <td className={`table-cell text-right font-semibold font-mono text-sm ${
                        isInvoice ? 'text-accent-600' : 'text-emerald-600'
                      }`}>
      ...
      {editModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/20 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl border border-gray-200 w-full max-w-lg shadow-xl max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-5 border-b border-gray-200">
              <div>
                <h3 className="text-base font-bold text-gray-900">Edit {editModal.docType}</h3>
                <p className="text-xs text-gray-500 font-mono mt-0.5">{editModal.docNo}</p>
              </div>
              <button onClick={() => setEditModal(null)} className="p-1 rounded-lg hover:bg-gray-100 text-gray-400 hover:text-gray-600">
                <X size={18} />
              </button>
            </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={formatDate(editModal.date)} readOnly
                    className="input-field bg-gray-50 text-gray-500 cursor-not-allowed text-sm" />
                </div>
                <div>
